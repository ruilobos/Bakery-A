...
| Deletion semantics | hard deletes everywhere | **Soft delete:** `Supplier`, `RawMaterial`, `Product`, `BaseRecipe`. **Hard delete:** ingredient lines. Tenant scope and archived state combine in **one** base manager (3.55) | [019](decisions.md) |
...
| Release tagging | `release.yml` on `main` (1.10), not yet run | **Every merge to `production` is tagged `vX.Y.Z` and published as a GitHub Release** with generated notes, by a GitHub Actions workflow using only the job's own token. The version is the release PR's title, `Release vX.Y.Z`, one of the three successors of the last tag; `0.y.z` until the pilot ships as `1.0.0` | [042](decisions.md) |
