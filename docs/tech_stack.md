...
| Auth/permissions | mix of no auth, `staff_member_required`, ad hoc template checks | consistent `LoginRequiredMixin` + real role-based permissions | Role **model** decided ([ADR-020](decisions.md)). **Authentication posture decided** ([ADR-027](decisions.md)): `LoginRequiredMiddleware` (Django 5.1) makes login the default for every view, with `@login_not_required` on the cover page and login view — forgetting a view is now safe rather than a silent hole (2.17). Still Open — 9.12: whether Django's built-in auth views replace the custom `user_login`, and the permission-class shape | Partly decided |
| Caching | none | cache expensive dashboard/costing aggregates — only if measurements show it's needed | — | Open — 9.18. **More likely to be needed now:** [ADR-023](decisions.md)'s overview dashboard aggregates across products against [ADR-021](decisions.md)'s p95 < 2 s budget, and [ADR-022](decisions.md)'s recursive costing makes each product's figure more expensive. [ADR-027](decisions.md) deliberately left this undecided rather than rejecting Django 4.0's built-in Redis backend: **decide it on 7.1's measurements**, not in advance, and don't let hosting cost decide it — a dashboard nobody waits for is worth more than the saving |
