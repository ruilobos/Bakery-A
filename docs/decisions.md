...
| [037](#adr-037-one-task--one-feature--one-pr) | Task-sized branches | Accepted (amends 010) | One task is one feature is one PR; `<epic-branch>-<task-id>` |
| [038](#adr-038-agent-assisted-task-loop) | Agent-assisted task loop | Accepted (amended by 039, 040) | `/next-task` executes one task and stops at the open PR |
| [039](#adr-039-rename-only-the-identifiers-epic-3-leaves-behind) | Prototype identifier renames | Accepted (amends 038) | Rename `Base_recipes` and `recipe_yeld` only; plain migration, no shims |
| [040](#adr-040-the-loop-verifies-its-premises-against-the-code) | Premise gate before planning | Accepted (amends 038) | A `codecheck` agent owns the code; the loop re-derives doc claims before they set scope |
...
- **2026-07-21 · Accepted** — **supersedes [001](#adr-001-branching-strategy-for-the-redesign)**; the persistent-staging clause is **amended by [014](#adr-014-local-docker-compose-is-the-devtest-environment)**; the **branch-granularity clause is amended by [037](#adr-037-one-task--one-feature--one-pr)** — one branch per *task*, not per plan. Everything else below stands.
...
## ADR-037: One task = one feature = one PR

- **2026-08-29 · Accepted** — **amends [010](#adr-010-main-as-integration-production-as-the-deploy-branch)'s branch-granularity clause only.**
- **Decision:** **One backlog task is one feature is one PR.** The branch is `<epic-branch>-<task-id>` (`phase-1-repo-cleanup-1.1`), squash-merged to `main`. **An epic is a unit of grouping and sequencing, never of branching.** Everything else in ADR-010 stands unchanged: `main` integrates, `production` deploys, every merge to `production` is tagged and released, no direct commits, both branches protected.
- **Rejected:** **Epic-sized branches** (ADR-010 as written) — Epic 3 is 74 tasks, which arrives as one unreviewable PR after weeks of nothing merging; the integration branch stops integrating. **Grouped sub-batches** — needs a per-epic judgment nobody has made, and the boundary gets re-argued every epic. **Trunk-based with direct commits to `main`** — already rejected by ADR-010 and nothing here changes that.
- **Traps:** **Roughly 20× more PRs across the backlog**, so PR creation has to be automated ([038](#adr-038-agent-assisted-task-loop)) or the ceremony costs more than the reviewability buys. **Task IDs are now branch names**, so the backlog's "never renumber" rule is load-bearing on git history too. A task too small to justify a branch is a sign it should have been a line inside another task — the fix is the backlog, not an exception here.

## ADR-038: Agent-assisted task loop

- **2026-08-29 · Accepted** — the 1.7 clause in Traps amended by [039](#adr-039-rename-only-the-identifiers-epic-3-leaves-behind); the loop definition amended by [040](#adr-040-the-loop-verifies-its-premises-against-the-code)
- **Decision:** `/next-task` is the sanctioned way to execute a backlog task: select it (**refusing any `Blocked` task**), read only the ADRs and requirement sections its Notes cite, plan, branch per [037](#adr-037-one-task--one-feature--one-pr), implement **with its tests**, review the diff against `decisions.md` *and* `project_requirements.md`, run the verification gates, update task status, commit, open the PR — and **stop**. It never merges to `main` and never promotes to `production`. **Three human gates: the plan, each commit's diff, the merge.** The tooling lives in `.claude/` and holds procedure and pointers only — never architectural facts, which stay in `docs/`.
- **Rejected:** **Agent teams** — experimental, and this backlog is serialized by epic dependencies, so parallel workers contend on the same files for no gain. **Autonomous merge** — removes the only review point before `main` exists as an integration branch worth trusting. **`claude -p` in CI** — nothing to gate on until a test suite and a host exist. **An MCP server for GitHub or Postgres** — `gh` is already authenticated with `repo` and `workflow` scopes, and there is no reachable database to introspect; **revisit Postgres at Epic 3**, where 74 tasks of schema work over 15 migrations would actually use it. **Importing the planning docs into `CLAUDE.md`** — imports load eagerly, adding ~46k tokens to every session; read-only subagents fetch the same answers on demand.
- **Traps:** **The loop is only as good as the Notes column** — a task citing no ADR gets reviewed against nothing, so a thin Notes cell is now a defect. **1.5 and 1.7 must route through plan mode**; they are refactors, not mechanical edits, and 1.7 touches 137 sites of `categorie` alone. **Guard hooks are local and invisible to anyone not using this tooling** — branch protection (1.9) and CI (1.11) remain the enforcement that actually binds; the hooks only make the failure arrive sooner.

## ADR-039: Rename only the identifiers Epic 3 leaves behind

- **2026-09-21 · Accepted** — amends [038](#adr-038-agent-assisted-task-loop)'s 1.7 clause
- **Decision:** Task 1.7 is rescoped to the two prototype identifiers **no other task renames** — `Base_recipes` → `BaseRecipe`, and `recipe_yeld` → `recipe_yield` on both `Base_recipes` and `Product`. It ships as a plain Django `RenameModel`/`RenameField` migration: **no transitional columns, no dual-write, no compatibility shim.** The other three are **deliberately not renamed**, because [032](#adr-032-one-shared-recipeline-and-categories-as-a-lookup-table) deletes them rather than respelling them: 3.74 drops the `categorie` column for the tenant-scoped `Category` table, and 3.73 merges `Bs_Ingredients` + `Recipe_Ingredients` out of existence. **Scope is the data layer only** — the model, the field, and the templates and admin that read them. The `Base_recipes_*` **view class** names are out of scope and stay as they are until 4.3, which settles view structure; renaming them would pull `urls.py` in for no schema benefit. Two corrections this rests on. **ADR-032 renames nothing** — it keeps `Base_recipes` a separate model and spells it the prototype way throughout, and 3.60 retypes `recipe_yeld` without respelling it, so absent 1.7 both misspellings reach the pilot permanently. **And the scale was never real:** `categorie` is a substring of `categories`, and this repo's `products_categories`/`rw_categories` URL names and per-template nav links inflate any count taken with a plain grep. ADR-038's Traps says "137 sites of `categorie` alone" and 1.15's Notes said 58 files; the field itself is **~22 sites in 6 files**, and 1.7 as rescoped touches **6 files and one migration** — `control/models.py`, `views.py`, `admin.py`, and the `base_recipe`, `products` and `new_product` templates.
- **Rejected:** **Transitional migrations and compatibility layers** — 1.7's original Notes, carried from the prototype and citing no ADR. Expand-and-contract protects old and new code running against one schema *simultaneously*. **That situation cannot arise yet, for the plain reason that there is no host** — Epic 12 has not started and 1.11's own Notes say "no deploy step (no host yet)"; `origin/production` exists only as the untouched prototype snapshot, 35 commits behind `main`, and 1.8 has still to adopt it as the deploy branch. Nothing is serving traffic to break. **The pre-deploy hook is not the argument** — [031](#adr-031-operations) completes `migrate` *before* the new version takes traffic, which for a rename leaves precisely the window where the old container meets the new column, so once there is a host this reasoning expires and a rename would need real care. It is the absence of a deployment, not the deploy mechanism, that makes a big-bang rename safe **today**. Supporting, not load-bearing: [021](#adr-021-non-functional-targets) sets no uptime target and [013](#adr-013-railway-hobby-plan-as-the-hosting-platform) states "no uptime SLA, no HA Postgres". A `RenameField` is an `ALTER TABLE … RENAME` and preserves every row. **Renaming all five now** — genuinely cheap at this scale and rejected on churn, not cost: it makes 3.19, 3.60, 3.73 and 3.74's text stale for three tables Epic 3 then deletes. **Folding both renames into Epic 3** — leaves the misspellings through Epics 2, 19 and 6, so every test written from 6.23 onward encodes them, and Epic 3 is four epics out. **Amending 3.73/3.74 to adopt the target names** — those tables are dropped, so the names would live only inside a migration. **Registering it as roadmap question 9.23** (opened 2026-09-18, withdrawn 2026-09-21 with its PR) — it recorded as an open judgment something already answerable from ADR-032 and the code; [roadmap.md](roadmap.md)'s own test is "a judgment nobody has made, or a clear path nobody has walked", and this was the second.
- **Traps:** **`Base_recipes` is referenced as a string** in `Bs_Ingredients.base_recipe`'s FK — the rename is not only the `class` statement, and a missed string reference fails at runtime, not at `check`. The URL *name* `control:base_recipe` is a routing label, not a model reference, and `RenameModel` does not touch it. **`recipe_yeld` exists on two models**, `Base_recipes` and `Product`; `export_products` names it in a `values_list` that is **already broken for an unrelated reason** (1.17), so rename it there without being drawn into that defect. **`RenameModel` renames the table**, so this migration stays separate from 3.73's merge rather than being squashed into it. **The three surviving misspellings are now deliberate** — that is what 1.15's rename guard is for: it stops them being "fixed" as a drive-by before 3.73 and 3.74 delete them. **A model renamed but its views not** is a half-finished rename on the face of it; it is intentional and 4.3 owns the rest, which is worth saying out loud in that task when it is planned.

## ADR-040: The loop verifies its premises against the code

- **2026-09-21 · Accepted** — amends [038](#adr-038-agent-assisted-task-loop)'s loop definition
- **Decision:** A **sixth subagent, `codecheck`**, owns the code as the five doc agents own their documents: the working tree and git history, never `docs/` for authority, returning every measurement **with the command that produced it**. `/next-task` gains a **premise gate that runs before planning**. Four kinds of claim are re-derived from the code before they are allowed to set a task's scope: **a number** (re-measure it); **"superseded by / absorbed into task N"** (confirm N produces the end state — search for the *target* spelling, not the current one); **a constraint** ("never X", "must use Y" — it must cite an ADR *and* name the condition it guards, and that condition must exist in this repo today); and **the task's purpose** (the outcome must still be something `project_requirements.md` or an ADR wants). A failed premise **stops the loop and is reported**, the same gate as `Blocked` and as a Notes cell citing no ADR. The `decisions` and `backlog` agents keep their read-only, one-document charters and **do not gain code access**; they instead flag any claim about the code as *stated when written, never re-measured*, and hand off to `codecheck`.
- **Rejected:** **Giving the five doc agents code access** — dissolves the context isolation [038](#adr-038-agent-assisted-task-loop) bought (it measured the alternative at ~46k tokens per session) and doubles every agent's working set for a check three of them would never run. **A CI check** — CI can re-run a count, but cannot verify "task N supersedes this one" or "this constraint guards a condition that does not exist"; those are readings of intent against code, not assertions a script can hold. **Relying on care rather than a gate** — the failure that prompted this survived four merged PRs, an accepted ADR's Traps and a Notes repair before [039](#adr-039-rename-only-the-identifiers-epic-3-leaves-behind) caught it, and two different readers reproduced the same wrong measurement independently. **A seventh "audit" agent sweeping all four documents for drift** — unbounded work with no task to scope it; the gate is affordable precisely because one task has few premises. **Re-measuring every figure in the backlog in one pass** — busywork ahead of need: a wrong number is harmless until it decides something, and the gate catches each one at exactly that moment. **Making it advisory** — a check the loop may skip is a check that gets skipped on the tasks that look obvious, which is the category this failure came from.
- **Traps:** **The gate must run before planning, not before implementing.** Its whole value is preventing work; a premise checked after the branch exists has already cost what it was meant to save. **`codecheck` must return the command, or it is one more unverified claim** — the substring bug behind this ADR was produced once and then *reproduced* by a second reader running the same wrong grep, and a recorded command is what breaks that loop. **Word boundaries and layer separation are the two rules that actually bite:** `categorie` matches `categories`, and a model name, a field name, a view class, a URL pattern name and a template variable routinely share a spelling while being different things. **A failed premise is a defect in the doc, not a licence to widen the task** — the repair is an ADR and a corrected line on its own PR. **This gate cannot catch a doc that is wrong about *intent*, only about fact** — a task can be accurate, unblocked and still pointless, which is why the fourth claim type reads requirements rather than code.

## ADR-041: &lt;next decision goes here&gt;
