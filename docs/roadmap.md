...
| 6 | Testing & quality gates | `phase-6-testing-ci` | Not started | Unit/integration tests, ruff/djlint, `pip-audit`, **extending** the Phase 1 minimal CI into the full pipeline, behind a **70% repo-wide coverage floor** | Builds on [decisions.md](decisions.md) ADR-011's Phase 1 foundation; gate shape fixed by **[ADR-031](decisions.md)** (9.14 ✅). The coverage gate is switched on at the **end** of the epic (6.22) — enabling it first blocks the PRs that write the tests. No SAST/container scanning this round |
| 7 | Observability & operations | `phase-7-observability` | Not started | Structured logging, **Sentry (EU region) error tracking with mandatory PII scrubbing**, health checks, **UptimeRobot uptime alerting**, deployment hardening | Tooling decided in **[ADR-031](decisions.md)** (9.15 ✅), both on free tiers. Note the sequencing: this epic depends on Epic 12 provisioning the host, so the pilot runs unmonitored until then |
...
| # | Phase | Branch | Status | Scope summary | Notes / links |
|---|---|---|---|---|---|
| 9 | Tech stack decisions | `stack-decisions` | **Done** (2026-08-26) | Resolve every `Open` row in [tech_stack.md](tech_stack.md); log each as an ADR | **Complete — all 22 tasks closed, and `tech_stack.md` has no `Open` rows left.** The runtime block closed 2026-08-10 ([ADR-025](decisions.md)/[ADR-026](decisions.md), creating Epic 19), the backend-architecture block 2026-08-13 ([ADR-028](decisions.md)), the dependency and frontend blocks 2026-08-16 ([ADR-029](decisions.md)/[ADR-030](decisions.md)), the operations block 2026-08-18 ([ADR-031](decisions.md)), and the final six on **2026-08-26** ([ADR-032](decisions.md)–[ADR-036](decisions.md)): a single shared `RecipeLine` plus a tenant-scoped `Category` table (9.18); the traceability event chain, `YYYYMMDD-NNN` lot codes and stock-derivable-but-not-surfaced (9.21); `ruff` + `pytest-django` (9.17); the tenant export as a CSV bundle + manifest (9.19); and Spark/Databricks **confirmed** on Serverless Jobs, AWS EU (Ireland), fed by a personal-data-free R2 extract under a ~$15/mo ceiling (9.20) — which released 9.4 (gunicorn, WSGI). **Epics 3, 4, 5, 6 and 12 are no longer blocked by this epic.** |
...
| 16 | AI insights & alerts service | `feature-ai-insights-service` | Not started | Margin alerts (notify when a product's margin drops below expectation) and analytics, computed from a nightly extract and returned to the app | **Engine confirmed** ([ADR-036](decisions.md), 9.20 ✅): **Databricks Serverless Jobs on AWS, EU (Ireland)** — [ADR-002](decisions.md) moves `Proposed` → `Accepted`, chosen over [ADR-024](decisions.md)'s scheduled-query null hypothesis as a deliberate bet on headroom. Django pushes a **personal-data-free** costing extract to R2; Databricks never touches PostgreSQL. Nightly schedule, results POSTed to a shared-secret callback — **no event triggering**. Still a **Could**, and now a **cost decision**: ~$6 → ~$21/mo total against zero revenue, so 16.8 measures a real run before production spend. Two new processors need DPAs (11.17, 16.4) |
| 17 | Batch & lot traceability | `feature-traceability` | Not started | Goods receipts with supplier lot codes, production runs consuming specific lots, internal batch lot codes, outbound records, and a one-step-back/one-step-forward trace query. Append-only records | **A legal obligation on the user** (EU Reg. 178/2002 Art. 18, FSAI-enforced), not a nice-to-have — see ADR-017 in [decisions.md](decisions.md). **Entity shape decided** ([ADR-033](decisions.md), 9.21 ✅): `GoodsReceipt`→`GoodsReceiptLine`, `ProductionRun`→`Consumption`→`Batch`→`OutboundRecord`, `YYYYMMDD-NNN` per-tenant lot codes, and stock derivable but deliberately not surfaced. Receipts-only was rejected on ADR-017's own "partial is worse than none". Still depends on Epic 3 (tenant scoping, numeric quantities, canonical units) and 10.8 (regulator check). Adds an event/temporal layer to a schema that is currently pure current-state — review Epic 3's design against it before implementing 3.19/3.22 |
