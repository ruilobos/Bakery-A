...
| 9.23 | Which of the prototype identifier renames survive Epic 3's schema work, and do they land before it or inside it? | **Blocks 1.7**, and sets what 1.15's rename guard is guarding. 1.7 renames five identifiers, but [ADR-032](decisions.md) mostly **deletes** them rather than renaming them: 3.73 merges `Bs_Ingredients` and `Recipe_Ingredients` into one `RecipeLine`, and ADR-032's trap says in terms that "**the migration is a merge, not a rename**"; 3.74 backfills a tenant-scoped `Category` table and **drops the `categorie` column** outright; 3.60 retypes `recipe_yeld` as a real numeric quantity with a unit, which [ADR-022](decisions.md) makes load-bearing. Only `Base_recipes`→`BaseRecipe` survives Epic 3 untouched, because ADR-032 keeps `Product` and `Base_recipes` separate models. So **four of the five renames would be edits Epic 3 then discards** — measured 2026-09-18: `categorie` 124 sites / 42 files, `Recipe_Ingredients` 46 / 10, `Base_recipes` 34 / 10, `recipe_yeld` 27 / 13, `Bs_Ingredients` 25 / 7. Candidate answers, none chosen: rescope 1.7 to `Base_recipes`→`BaseRecipe` only and fold each remaining rename into the Epic 3 task that already rewrites that field; rename all five now regardless, buying a readable codebase through Epics 2 and 19 at the price of migrations 3.73 and 3.74 unwind; or close 1.7 and let every rename arrive with Epic 3's schema. **The answer decides whether 1.7 is a task, three lines inside other tasks, or nothing** — and 1.7 cites no ADR today, which is the defect that surfaced it. Surfaced 2026-09-18 while planning 1.7 |
