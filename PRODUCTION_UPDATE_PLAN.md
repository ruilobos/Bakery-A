...
| ID | Task | Status | Notes / source |
|---|---|---|---|
| 1.5 | Split settings into `settings/base.py` + `local.py` + `test.py`; retire the Heroku-specific module | Not started | **Unblocked and rescoped by [ADR-028](docs/decisions.md)** (9.9 ✅): **three** modules, not four — `base.py` *is* the production configuration, so there is no `production.py` and nothing in the deploy path is env- or host-named ([ADR-015](docs/decisions.md) rule 2). `manage.py` and the `Dockerfile` both default to `base`. Pairs with 2.1 (secrets to env) and 2.19 (fail loudly when they are missing); deletes `settings/heroku.py` (7.9) and `runtime.txt` |
...
| ID | Task | Status | Notes / source |
|---|---|---|---|
| 2.10 | Replace the `print()` login diagnostics in `accounts/views.py` with structured logging that never records passwords | **Superseded** by 4.5 + 2.23 | **Severity downgraded by [ADR-028](docs/decisions.md)** — verified 2026-08-13: nothing includes `accounts.urls` and no template reverses `user_login`, so the password-logging `print()` is **unreachable dead code, not a live leak**. It was never the "highest-priority security defect"; the live login path has always been Django's `LoginView`. The code is **deleted** by 4.5, and the genuine requirement it gestured at — auditing failed logins without recording credentials — becomes 2.23. Still worth a line in [gdpr.md](docs/gdpr.md) §6 recording that it never executed |
...
| 2.19 | Make `settings/base.py` fail loudly at boot when `SECRET_KEY`, `ALLOWED_HOSTS` or `DEBUG` is unset — no defaults for any of the three — and cover it with a test | Not started | [ADR-028](docs/decisions.md) (9.9 ✅). The point of the layout: a missing variable raises `ImproperlyConfigured` instead of silently running on dev defaults. Directly retires today's inversion, where `base.py` ships a real `SECRET_KEY`/`DEBUG = True` and `heroku.py:11` defaults `DEBUG` to `True` in the *production* module. Pairs with 1.5, 2.1 |
| 2.20 | Define the capability set per role as permission codenames, and implement the custom authentication backend that resolves `has_perm()` against the active membership | Not started | [ADR-028](docs/decisions.md) (9.12 ✅), on [ADR-020](docs/decisions.md)'s three roles. Capability-named, never `role == "owner"`. Keeps `PermissionRequiredMixin`, `@permission_required` and `{% if perms.… %}` working — **one** permission vocabulary. **Reshapes 2.7**: the "explicit permission classes" become stock Django mixins over these codenames. Needs 3.58 |
| 2.21 | Add middleware resolving the active bakery + membership onto the request, and invalidate Django's cached permissions whenever the active tenant changes | Not started | [ADR-028](docs/decisions.md). The invalidation half is a **cross-tenant leak** if missed — a user with two memberships would carry bakery A's capabilities into bakery B. Exactly what [ADR-008](docs/decisions.md)'s dedicated isolation tests exist for; add a test with a two-membership user |
| 2.22 | Add brute-force protection to the login endpoint (attempt throttling with lockout/backoff) | Not started | **Gap surfaced by [ADR-028](docs/decisions.md)** — no epic covered it. Django's auth views ship no rate limiting, and adopting them as the only auth surface (4.5) makes this the single credential entry point for every tenant |
| 2.23 | Emit structured audit events for failed and successful logins that record the attempt but **never** the submitted credentials | Not started | **Replaces 2.10**, which described dead code. [gdpr.md](docs/gdpr.md) §6. Pairs with 2.22 (throttling needs the attempt record) and feeds 11.7's DPIA, since login history is personal data with its own retention question |
...
| ID | Task | Status | Notes / source |
|---|---|---|---|
| 3.19 | Build the shared `RecipeLine` model — typed parent (`parent_product` XOR `parent_base_recipe`), typed component (`component_material` XOR `component_recipe`), `quantity` + unit FK — replacing both `Bs_Ingredients` and `Recipe_Ingredients` | **Unblocked** | [ADR-032](docs/decisions.md) (9.18 ✅). Both XOR pairs are database `CHECK` constraints written in the same migration, **not** `clean()` — `clean()` does not run on `bulk_create`, which is exactly how an import will write these rows. Merge migration and constraints are 3.73 |
...
| 3.26 | Standardize units and categories via controlled enums or lookup/reference tables | **Unblocked** | **Both are lookup tables.** Units ([ADR-018](docs/decisions.md)) are built in 3.52; categories ([ADR-032](docs/decisions.md)) in 3.74 — one tenant-scoped `Category` table with a `kind` discriminator. The difference that matters: categories are tenant-editable, units are not |
...
| 3.38 | Create backup, restore, and retention procedures — including a tested restore drill | Not started | **Unblocked** by [ADR-031](docs/decisions.md) (9.16 ✅): *backup* retention is fixed at 7 daily / 4 weekly / 12 monthly. That is a **different clock** from GDPR *record* retention, which stays `Open` in [gdpr.md](docs/gdpr.md) §5 (11.4) and the food-law floor in 10.9 — those constrain what the database holds, not how long a dump of it survives. Executed by 3.70–3.72 and 3.44–3.48; written up in 8.5 |
...
| 3.40 | Enable automated backups; **PITR is deliberately not adopted** | Not started | [ADR-031](docs/decisions.md) (9.16 ✅). Railway offers no PITR, and WAL archiving via `pgBackRest`/`wal-g` was rejected as a self-managed component protecting a megabyte-scale dataset that is re-enterable from paper invoices. **RPO of up to 24 h is an accepted, stated position** — record it in 8.5 so the pilot bakery has seen it. Revisit at the second paying tenant. Executed by 12.9 (native schedules) + 3.70 (the portable dump) |
| 3.41 | Separate application and database credentials per environment | Not started | |
| 3.42 | Add connection pooling if traffic or deployment topology requires it | **Superseded by 7.19** | Closed by [ADR-027](docs/decisions.md) — Django 5.1's native psycopg pool plus `CONN_HEALTH_CHECKS`, no separate pooler service. The `9.18` blocker on this row was stale; the work lives in 7.19 |
...
| 3.48 | Automate the restore drill **weekly** — decrypt the latest dump, restore it into a throwaway PostgreSQL 17, run 3.47's checks, and alert on failure | Not started | **The key one.** App migration is low-risk because deploys rehearse it constantly; a database restore path that runs once a year is where the surprises live. Cadence and the decrypt step fixed by [ADR-031](docs/decisions.md): the drill is the **only** recurring proof that 3.72's `age` key still works, so it must decrypt rather than restore a plaintext copy. Alerting on a *missed* run matters as much as on a failed one |
...
| 3.70 | Build the nightly backup job as a scheduled GitHub Actions workflow: `pg_dump -Fc --no-owner --no-privileges` → `age` encrypt → upload to R2 | Not started | [ADR-031](docs/decisions.md) (9.16 ✅). Uses 3.46's committed scripts rather than inlining flags in the workflow. **Runs outside Railway on purpose** — an escape hatch that depends on the platform it escapes is not one. Needs backup-scoped DB credentials (3.41) reaching Postgres over Railway's public TCP proxy, and R2 credentials scoped to write-only on the backup bucket. Must fail loudly: a silently skipped nightly job is indistinguishable from a working one until the day it matters |
| 3.71 | Provision the backup R2 bucket — **separate from the media bucket** — with a lifecycle rule keeping 7 daily, 4 weekly, 12 monthly | Not started | [ADR-031](docs/decisions.md). Separate bucket so media credentials can never read or delete backups, and so [ADR-005](docs/decisions.md)'s media lifecycle stays independent of this one. A few hundred MB sits inside R2's 10 GB free tier. The 12 monthlies are also the outer bound on how long deleted personal data survives in restorable form — feeds [gdpr.md](docs/gdpr.md) §5 and 11.4 |
| 3.72 | Generate the `age` key pair, publish the **public** key to CI, and store the private key outside CI with a documented recovery path | Not started | [ADR-031](docs/decisions.md). CI only ever needs the public key, so a compromised runner cannot read any backup. **The private key has no software fallback** — lose it and every dump in R2 is unrecoverable, which is exactly why 3.48's weekly drill must decrypt rather than merely restore. Custody and recovery go in the 8.5 runbook and the 8.6 rotation runbook |
| 3.73 | Migrate `Bs_Ingredients` + `Recipe_Ingredients` into the single `RecipeLine` table, add both XOR `CHECK` constraints, and drop the old tables only after row counts reconcile | Not started | [ADR-032](docs/decisions.md). A merge, not a rename: two tables of live prototype data fold into one with a discriminating parent column. The constraints ship in the same migration as the model (3.19) |
| 3.74 | Build the tenant-scoped `Category` table (`kind` discriminator, `archived`, unique on `(bakery, kind, name)`), backfill distinct free-text `categorie` values per tenant, then drop the column | Not started | [ADR-032](docs/decisions.md). Resolves the categories half of 3.26/9.18, mirroring what 3.52 does for units. **`kind` must be validated against the referencing model** — the FK cannot stop a product being filed under a flour category, so the form and service layer must |
...
| ID | Task | Status | Notes / source |
|---|---|---|---|
| 4.1 | Move costing/margin calculations into `control/services/`, with one implementation — deleting both the model `@property` methods and the inline `float()` loops in `control/views.py` | Not started | **Shape fixed by [ADR-028](docs/decisions.md)** (9.10 ✅): plain module-level functions grouped by concern (`costing`, `pricing`, `receipts`), explicit arguments, frozen dataclass returns. Models keep fields/constraints/managers only — the properties are **deleted, not delegated**. The inline `float()` math is deleted rather than ported ([ADR-018](docs/decisions.md)). Build 4.19 first |
...
| 4.5 | Delete the dead custom auth code — `accounts/views.py` and `accounts/urls.py` — leaving Django's auth views as the only auth surface | Not started | **Unblocked by [ADR-028](docs/decisions.md)** (9.12 ✅), and **smaller than recorded**: `bakery/urls.py` includes `django.contrib.auth.urls` but **never includes `accounts.urls`**, so `user_login` is unreachable and even renders a template path that does not exist. A pure deletion with no behaviour to port — **no longer depends on 2.10**. Keep `accounts/templates/registration/` |
...
| 4.14 | Cache expensive dashboard summaries | **Cancelled** | [ADR-028](docs/decisions.md) — **no cache in the first release**: a stale margin is not a slow page, it is a wrong number someone prices against, and the pilot is one bakery whose ingredient graph costs in milliseconds once batched (4.19). Escalation ladder gated on 7.20's measurements; version-stamp seam built into the batch service (4.20). The `9.18` blocker on this row was stale |

### API readiness

| ID | Task | Status | Notes / source |
|---|---|---|---|
| 4.15 | ~~Build the agreed API surface (DRF or a small read-only reporting/export API), versioned from the first endpoint~~ | **Cancelled** (2026-08-13) | **No API layer** — [ADR-028](docs/decisions.md) (9.11 ✅). Every candidate consumer is excluded, and the health/callback endpoints are plain `JsonResponse` views needing no framework. Cancelled rather than deferred; the ADR carries the revisit trigger, and the framework gets chosen if and when it fires |
...
| 4.19 | Build the `control/services/` package skeleton: `cost_products(queryset, …)` as the **primary batch entrypoint** loading the whole ingredient graph in a bounded number of queries, single-object costing as a thin wrapper, and a frozen `CostBreakdown` dataclass carrying amount, canonical unit, provenance and as-of date | Not started | [ADR-028](docs/decisions.md) (9.10 ✅). Batch-first is not a preference — [ADR-023](docs/decisions.md)'s dashboard costs every product per request against [ADR-021](docs/decisions.md)'s p95 < 2 s, and it is also **why no cache is needed** (9.18). Carries [ADR-022](docs/decisions.md)'s recursion, cycle guard and depth limit. Prerequisite for 4.1 |
| 4.20 | Give the batch costing signature a per-tenant version-stamp parameter, bumped by writes to costing inputs, so a cache can later be attached at that seam without redesigning the API | Not started | [ADR-028](docs/decisions.md) — cheap insurance behind the "no cache in first release" decision. Not wired to any cache now; the stamp exists so escalation step (b) is a configuration change rather than a refactor if 7.20 measures a miss. Inputs that must bump it: goods receipts, recipe edits, cascading base-recipe edits, VAT rows, `ProductPrice` rows |
...
| ID | Task | Status | Notes / source |
|---|---|---|---|
| 5.1 | Introduce a shared `base.html` layout | Not started | [ADR-030](docs/decisions.md). **31 of 32 templates carry their own full `<!DOCTYPE>` boilerplate** and there is not one `{% extends %}` in the repo — this is the highest-leverage task in the epic |
| 5.2 | Replace duplicated navbar/footer/page scaffolding with reusable template blocks and partials in `<app>/templates/partials/` | Not started | [ADR-030](docs/decisions.md). The navbar is duplicated across **all 32** templates. Plain `{% include %}` — `django-template-partials` is deliberately not adopted unless HTMX fragments outgrow a handful |
| 5.3 | Replace hardcoded static paths with `{% load static %}` / `{% static %}` | Not started | **Bigger than it looks, and it must be sequenced with 1.3.** Every template in the repo hardcodes `/static/...` — there is no `{% static %}` in any project template — which means whitenoise's manifest hashing has never done anything here. Switching to `{% static %}` activates it, and `ManifestStaticFilesStorage` **raises `ValueError` at render time** for any referenced file missing from the manifest: a missing asset stops being a silent 404 and becomes a 500. Do it **after 1.3** removes the committed `bakery/staticfiles/` output, so a stale manifest can't be the cause, and lean on 1.13's CI `collectstatic`. Related: 19.14, [ADR-027](docs/decisions.md). **[ADR-030](docs/decisions.md) raises the stakes again:** with no build step, whitenoise is now the *only* cache-busting mechanism, so 5.3 + 19.14 + 1.13 are the whole story on asset versioning — done partially, the site silently serves stale CSS |
| 5.4 | Update the vendored Bootstrap from 5.0.0 to current 5.3.x | **Unblocked** | [ADR-030](docs/decisions.md). Same major version, so this is an upgrade rather than a restyle — but verify the components actually in use (navbar, tables, forms, buttons) render unchanged. The vendored set also includes an RTL build and `.map` files that nothing references; leave or drop them as a judgment call, not scope |
| 5.5 | Delete the seven empty JavaScript files and the `<script>` tags referencing them | Not started | Verified 2026-08-16: **all seven are 0 bytes**, so there is nothing to preserve — this is a deletion, not a rewrite. [ADR-030](docs/decisions.md)'s interactivity layer starts from zero, which is what makes the no-bundler decision defensible |
| 5.6 | **Rescoped — no build step.** Consolidate the 16 custom CSS files (922 lines, four of them byte-identical) into a small set built on CSS custom properties, and rely on whitenoise's manifest storage for hashing and compression | **Unblocked** | [ADR-030](docs/decisions.md) **rejects the Vite candidate** — 0 bytes of custom JS and 922 lines of CSS do not justify Node, a second lockfile and CI steps to duplicate what 19.14 already does. Native nesting/custom properties are safe on [ADR-021](docs/decisions.md)'s evergreen matrix. Revisit trigger in the ADR |
| 5.7 | Add HTMX as the progressive-enhancement layer, and settle the fragment convention: on `HX-Request`, render the partial directly rather than the full page | **Unblocked** | [ADR-030](docs/decisions.md). Server returns HTML fragments — what [ADR-028](docs/decisions.md) assumed when it rejected an API layer. **Every form and link must still work with JS disabled** — required by WCAG 2.2 A ([ADR-021](docs/decisions.md), 5.12) regardless. Settle the convention *before* 5.9 is built, not during it |
| 5.8 | Add `djlint` for template linting/formatting | **Unblocked** | [ADR-030](docs/decisions.md) closes the template half of 9.17. A pip package, joining `ruff`/`pytest` in [ADR-029](docs/decisions.md)'s `requirements/dev.in`, with no Node in dev. Standalone CSS/JS linting (`stylelint`/`prettier`) stays **deferred** on the same no-Node grounds as the asset pipeline — reconfirmed by [ADR-034](docs/decisions.md), not left open |
...
| 5.24 | Wire up and template Django's password-reset flow (the four `registration/password_reset_*` templates plus the two email bodies), styled with the rest of Epic 5 | Not started | [ADR-028](docs/decisions.md) — a **direct consequence of adopting Django's built-in auth views** (9.12 ✅): password reset is email-only, so this is the account-recovery path for every user. Needs 7.21's email configuration. Use [ADR-027](docs/decisions.md)'s `as_field_group()` rendering (5.22) and `gettext` on the copy (5.15) |
| 5.25 | Record every vendored frontend asset — Bootstrap and HTMX — with its exact version, source URL and purpose, and check it on the same cadence as the dependency recompile | Not started | [ADR-030](docs/decisions.md) extends [ADR-029](docs/decisions.md)'s owner/purpose rule to assets `requirements/*.in` cannot see. Without it the repo reproduces the state this decision found: **Bootstrap 5.0.0 from May 2021, four years stale, unnoticed**. Pairs with 5.4 and 5.7 |
...
| 6.22 | Add coverage measurement to the CI workflow, then enable `--fail-under=70` as a required check | Not started | [ADR-031](docs/decisions.md) (9.14 ✅). **Enable the gate last, at the end of this epic** — switching it on first blocks the very PRs that write the tests. Measure and report from the start so the number is visible while it climbs. Repo-wide, one number: a per-module scope was rejected as an invitation to argue about which module counts |

### Tooling

| ID | Task | Status | Notes / source |
|---|---|---|---|
| 6.10 | Add and configure the agreed formatter/linter across the codebase | **Unblocked** | **`ruff check` + `ruff format`** — [ADR-034](docs/decisions.md) (9.17 ✅). No `black`, no `isort`. Config in `[tool.ruff]` of the tool-only `pyproject.toml`; enters `requirements/dev.in` with an owner comment per [ADR-029](docs/decisions.md) |
| 6.11 | Add template linting | **Unblocked** | **Adopted: `djlint`** ([ADR-030](docs/decisions.md), reconfirmed by [ADR-034](docs/decisions.md)). The tooling work itself is 5.8 — this row is now only wiring it into the CI gate (6.12) |
| 6.23 | Adopt `pytest` + `pytest-django` + `pytest-cov` as the test runner, with `[tool.pytest.ini_options]` in the tool-only `pyproject.toml` | Not started | [ADR-034](docs/decisions.md) (9.17 ✅). `--cov-fail-under=70` is where [ADR-031](docs/decisions.md)'s floor is actually enforced, which keeps 6.22 a one-line change. `pytest-django` wraps Django's test-database machinery rather than replacing it |
| 6.24 | Update the documented test command from `python manage.py test` to `pytest` in `CLAUDE.md`, the README (8.1) and the CI workflow | Not started | [ADR-034](docs/decisions.md). Three places, and the stale one is always the one someone follows |

### CI/CD

| ID | Task | Status | Notes / source |
|---|---|---|---|
| 6.12 | Extend the Epic 1 workflow (do not replace it) with: dependency install from the hashed lock, the real test suite, `ruff` + `djlint`, and `pip-audit` | Not started | [ADR-011](docs/decisions.md), shape fixed by [ADR-031](docs/decisions.md) (9.14 ✅). `pip-audit` runs against [ADR-029](docs/decisions.md)'s `requirements/*.txt`. **No CodeQL/Trivy** this release — the revisit trigger is in the ADR. Coverage is 6.22, deliberately separate |
| 6.13 | Enable required status checks on `main` and `production`, blocking merge when checks fail | Not started | [ADR-010](docs/decisions.md). **This is also the deploy gate** — see 6.14 |
| 6.14 | Block deployments when checks fail | Not started | **Satisfied by 6.13, not by a second mechanism** — [ADR-031](docs/decisions.md). `production` only advances by merge and Railway deploys only what lands there, so a blocked merge is a blocked deploy. The work here is *verifying* that chain end to end (attempt a failing merge, confirm no deploy fires), not building a pipeline step |
...
| 7.2 | Add error tracking: create the Sentry organisation **in the EU region**, add `sentry-sdk[django]` to `requirements/base.in` with its owner comment, and wire the DSN in from the environment | Not started | [ADR-031](docs/decisions.md) (9.15 ✅). **The region is chosen at org creation and cannot be changed afterwards** — get it wrong and the fix is a new organisation. DSN follows [ADR-015](docs/decisions.md) rule 2 (env var, in the 7.13 `.env.example`); tag environment and release so pilot noise stays separable. PII handling is 7.23 and is **not optional** |
...
| 7.5 | Add uptime monitoring and alerting: a 5-minute UptimeRobot **keyword** check against 7.4's health endpoint, with email alerting | Not started | [ADR-031](docs/decisions.md) (9.15 ✅). Keyword match on the response body, **not** HTTP 200 alone — otherwise a health endpoint that correctly reports a dead database still passes. Hosted away from Railway on purpose. Needs 7.4 to return something a keyword check can assert on |
...
| 7.14 | Run the app under Gunicorn (or the process manager appropriate to the chosen host) in production | **Unblocked** | **gunicorn, WSGI, sync workers**, latest pinned at lock time — [ADR-036](docs/decisions.md) (9.4 ✅). Binds `$PORT` per 7.17 and [ADR-015](docs/decisions.md) rule 3. Revisit only if the insights dashboard needs SSE/WebSocket push rather than polling |
| 7.15 | Automate deploys: Railway git-watch on `production` only — there is no hosted staging environment to deploy to | Not started | [ADR-010](docs/decisions.md), narrowed by [ADR-014](docs/decisions.md) (the `main` tier is local) and settled by [ADR-031](docs/decisions.md) (9.14 ✅). **Unblocked** — 12.1 chose the host. Platform-native git-watch, not a GitHub Actions deploy job: that would need a long-lived Railway token in CI and a release script to maintain. Migrations are 12.11, not part of the deploy command |
...
| 7.20 | Enable and use `pg_stat_statements` for slow-query visibility, and check the [ADR-021](docs/decisions.md) budgets against it (p95 < 500 ms pages, < 2 s dashboard) | Not started | [ADR-027](docs/decisions.md), PostgreSQL 17 — split shared/local block I/O timing plus `stats_since`. Nothing currently measures those budgets. Feeds 3.43 and 5.14. **The named trigger** for [ADR-028](docs/decisions.md)'s caching escalation ladder: if the dashboard misses p95 < 2 s on realistic pilot data, work the ladder in order — fix the queries, then a version-keyed per-view cache (4.20), then Redis only for cross-worker coherence |
| 7.21 | Configure email from environment variables: Django's built-in `smtp.EmailBackend` against Brevo in `base.py`, console backend in `local.py`, locmem in `test.py`, with `DEFAULT_FROM_EMAIL` set | Not started | [ADR-028](docs/decisions.md) (9.22 ✅). No dependency — the provider stays swappable by configuration alone ([ADR-015](docs/decisions.md) rule 2). Variables go in the `.env.example` from 7.13; credentials follow 2.1. Blocks 5.19 (invitations) and 5.24 (password reset) |
| 7.22 | Create the Brevo account, verify the sending domain, and publish SPF, DKIM and DMARC records for it | Not started | [ADR-028](docs/decisions.md). The half that is not code and is the usual cause of "the invitation never arrived" — unauthenticated mail from a new domain lands in spam regardless of the application being correct. Do before 5.19 ships to a real pilot user |
| 7.23 | Configure Sentry PII handling: `send_default_pii=False` plus a `before_send` scrubber stripping request bodies, headers, cookies and local variables | Not started | [ADR-031](docs/decisions.md). **The GDPR-load-bearing half of 7.2.** A traceback from this app can carry supplier contact details, staff emails and a tenant's costing data; shipping those to a processor by accident is a personal-data disclosure no DPA covers. Test it — trigger a deliberate exception on a view handling personal data and inspect what actually arrived. Pairs with 11.6 and 2.9/7.3's "no secrets in logs" rule |
...
**Epic 9 is complete as of 2026-08-26.** All 22 tasks are `Done` and
[tech_stack.md](docs/tech_stack.md) has no `Open` rows left. The final six closed together in
[ADR-032](docs/decisions.md)–[ADR-036](docs/decisions.md): the ingredient-line and category shapes
(9.18), the traceability entities, lot codes and stock (9.21), Python lint/test tooling (9.17), the
tenant export format (9.19), and the insights engine with its cost and data-handover model (9.20) —
which finally released 9.4, deliberately held for it since [ADR-025](docs/decisions.md). Epics 3, 4,
5, 6 and 12 are no longer blocked by this epic. A new stack question re-opens it with a new task
here *and* an `Open` row in `tech_stack.md`; it does not get answered in passing.
...
| 9.4 | Decide the WSGI/ASGI server and version | **Done** (2026-08-26) | **gunicorn, WSGI, sync workers**, latest pinned at lock time — [ADR-036](docs/decisions.md), decided with 9.20 as planned. Nothing in the confirmed insights shape is async: a nightly extract command and a short callback POST. One revisit trigger — the insights dashboard needing SSE/WebSocket push rather than polling. Unblocks 7.14 |
...
| 9.8 | Decide the dependency management approach (requirements split + lock workflow), and confirm every remaining dependency has an owner and purpose | **Done** (2026-08-16) | **`uv pip compile` over `requirements/{base,dev,prod}.in` → pinned, hashed `.txt`** — [ADR-029](docs/decisions.md). Requirements-file mode, not `pyproject.toml`, so the install path is unchanged everywhere. The **`.in` file is the dependency register**: every line carries an owner/purpose comment or it goes at the next recompile. **Unblocks 19.12**; creates 19.16, 19.17; **amends [ADR-025](docs/decisions.md)** — `Pillow` is removed (no `ImageField` exists) and returns in 13.1 |
| 9.9 | Decide the settings module layout | **Done** (2026-08-13) | **`base.py` + `local.py` + `test.py`, no `production.py`** — [ADR-028](docs/decisions.md). `base.py` *is* production: env-driven, no default for `SECRET_KEY`/`ALLOWED_HOSTS`/`DEBUG` so a missing var fails at boot, and a forgotten `DJANGO_SETTINGS_MODULE` lands on the safe module. Unblocks and **rescopes** 1.5 (three modules, not four); creates 2.19 |
| 9.10 | Decide where business logic lives (services/domain module shape) | **Done** (2026-08-13) | **`control/services/` — plain functions, frozen dataclass returns, batch-first** — [ADR-028](docs/decisions.md). Models lose costing entirely. Decided by [ADR-023](docs/decisions.md)'s dashboard needing batch costing against [ADR-021](docs/decisions.md)'s budget, which a per-instance `@property` cannot do. Unblocks 4.1; creates 4.19, 4.20 |
| 9.11 | Decide whether a JSON API is needed at all, and if so which framework | **Done** (2026-08-13) | **No API layer, no framework pre-selected** — [ADR-028](docs/decisions.md). Every candidate consumer is excluded ([ADR-021](docs/decisions.md) rules out a native app; the frontend is server-rendered; the insights service has Django as the client). Health/callback endpoints are plain `JsonResponse` views. **Cancels 4.15**; revisit trigger in the ADR |
| 9.12 | Decide the auth/permissions approach (Django auth views as standard, permission model shape) | **Done** (2026-08-13) | **Django's built-in auth views only; capabilities as permission codenames resolved by a custom auth backend** — [ADR-028](docs/decisions.md), completing [ADR-020](docs/decisions.md)/[ADR-027](docs/decisions.md). `accounts.views.user_login` is **unreachable** (`accounts.urls` is never included), so 4.5 is a pure deletion and 2.10's leak is dead code. Creates 2.20–2.23 |
| 9.13 | Decide the frontend stack: templating approach, CSS framework/version, asset pipeline, interactivity layer, and whether an SPA is needed | **Done** (2026-08-16) | **Shared `base.html` + `{% include %}` partials, Bootstrap 5.3.x, HTMX, no SPA — and deliberately no build step** — [ADR-030](docs/decisions.md). Decided against a **measured** frontend: zero `{% extends %}`/`{% include %}` in 4,277 template lines, zero `{% static %}`, Bootstrap 5.0.0, and **0 bytes of custom JS** — which is what rejects the Vite candidate. Unblocks 5.4, 5.6, 5.7; rescopes 5.6; creates 5.25; also closes the template half of 9.17 (5.8) |
| 9.14 | Decide the CI/CD pipeline shape beyond the Epic 1 minimum | **Done** (2026-08-18) | **Tests + `ruff`/`djlint` + `pip-audit` + a 70% repo-wide coverage floor, extending 1.11 rather than replacing it; deploys stay Railway git-watch with `migrate` as the pre-deploy command** — [ADR-031](docs/decisions.md). No SAST/image scanning this release (revisit: repo goes public, first non-pilot tenant, or payment data). 6.14 is satisfied by **branch protection**, not a second gate. Unblocks 6.12, 7.15; creates 6.22, 12.11 |
| 9.15 | Decide error tracking and uptime monitoring tooling | **Done** (2026-08-18) | **Sentry SaaS free tier in the EU region (`de.sentry.io`) + UptimeRobot free tier** — [ADR-031](docs/decisions.md). Both $0. The Sentry region is **irreversible after org creation**; `send_default_pii` off and `before_send` scrubbing are part of the decision, not tuning. GlitchTip rejected on cost (its own container *and* Postgres ≈ doubles the bill) but stays a configuration-level fallback — same SDK. Unblocks 7.2, 7.5; creates 7.23; adds Sentry to 11.6 |
| 9.16 | Decide the backup/restore/PITR strategy — must cover **both** the host's native snapshots (fast rollback) and a portable logical dump (host replaceability) | **Done** (2026-08-18) | **Two unequal tracks, no PITR** — [ADR-031](docs/decisions.md). (a) Railway native daily/weekly/monthly for same-host rollback (12.9); (b) a nightly GitHub Actions `pg_dump -Fc --no-owner --no-privileges`, **`age`-encrypted client-side**, into R2 under a 7-daily/4-weekly/12-monthly lifecycle rule — outside Railway by design ([ADR-015](docs/decisions.md) rule 5). Proven weekly by 3.48. **RPO up to 24 h accepted**; revisit at the second paying tenant. Unblocks 3.38, 3.40, 3.44; creates 3.70–3.72 |
| 9.17 | Decide formatting/linting and testing tooling (~~including template linting~~) | **Done** (2026-08-26) | **`ruff` for lint *and* format; `pytest` + `pytest-django` + `pytest-cov` as the runner** — [ADR-034](docs/decisions.md). No `black`/`isort`. Django's own runner lost on this suite's shape: table-driven costing (`parametrize`) and composable tenant-isolation fixtures. Tool config in a `[tool.*]`-only `pyproject.toml` that declares no dependencies, so [ADR-029](docs/decisions.md) stands. Unblocks 6.10, 6.11; creates 6.23, 6.24 |
| 9.18 | Decide the outstanding data-model questions: ingredient-line model shape, categories as enums vs. lookup tables, ~~dashboard caching~~, ~~connection pooling~~ | **Done** (2026-08-26) | **One shared `RecipeLine` (typed parent XOR, typed component XOR, both as database `CHECK` constraints); categories as one tenant-scoped `Category` table with a `kind` discriminator** — [ADR-032](docs/decisions.md). `Product` and `Base_recipes` stay separate models: this collapses the *lines*, not the recipes. Categories are tenant-editable (they carry no arithmetic); units are **not** (a conversion factor rewrites every derived cost). Caching and pooling were already closed by [ADR-028](docs/decisions.md)/[ADR-027](docs/decisions.md). Unblocks 3.19 and the categories half of 3.26; creates 3.73, 3.74; **corrects 3.42 and 4.14**, which still cited this task as their blocker |
| 9.19 | Decide the tenant export format and generation mechanism | **Done** (2026-08-26) | **A ZIP of one CSV per tenant-scoped table plus a `manifest.json`** (timestamp, schema version, columns, types, PK, FK map), generated by one service function exposed as an Owner-only settings download *and* a management command — [ADR-035](docs/decisions.md). ANSI SQL rejected as a type-map/quoting burden serving only a developer; `dumpdata` rejected as a Django format, the exact dependency [ADR-008](docs/decisions.md) existed to avoid. **The manifest is the deliverable** — CSV loses types, null-vs-empty and every relationship. Unblocks 14.1; creates 14.6 |
| 9.20 | Re-confirm Spark/Databricks against a plain-Python batch alternative at this data scale, and confirm the workspace cost model | **Done** (2026-08-26) | **Confirmed — [ADR-002](docs/decisions.md) moves `Proposed` → `Accepted`**, detailed by [ADR-036](docs/decisions.md): **Databricks Serverless Jobs on AWS, EU (Ireland)**, fed by a personal-data-free R2 extract (never direct JDBC to Postgres), on a nightly schedule with results POSTed to a shared-secret callback, under a hard **~$15/mo** ceiling. A deliberate bet on headroom over [ADR-024](docs/decisions.md)'s scheduled-query null hypothesis, with the cost (~$6 → ~$21/mo total) and the two new processors accepted up front. **Also closes 9.4.** Unblocks 16.1–16.3; creates 16.8–16.13, 11.17 |
| 9.21 | Decide the traceability data-model shape: goods-receipt/batch/production-run entities and their relationships, internal lot-code generation strategy, and whether stock/quantity-on-hand comes along as a derived by-product or is deliberately excluded | **Done** (2026-08-26) | **Header + lines, full event chain; `YYYYMMDD-NNN` per-tenant lot codes; stock derivable but not surfaced** — [ADR-033](docs/decisions.md). `GoodsReceipt`→`GoodsReceiptLine`, `ProductionRun`→`Consumption`→`Batch`→`OutboundRecord`. Receipts-only was rejected on [ADR-017](docs/decisions.md)'s own "partial is worse than none". `Consumption.quantity_consumed` is the single field that keeps on-hand derivable later without a schema change. **Decided before Epic 3 starts, which was the point.** Unblocks 17.1–17.3; creates 17.13, 17.14 |
| 9.22 | Choose an email provider and integration approach — required before user invitations can ship | **Done** (2026-08-13) | **Brevo over SMTP**, Django's built-in `smtp.EmailBackend`, no dependency — [ADR-028](docs/decisions.md). EU-owned and EU-hosted, ISO 27001:2022, DPA by default, 300/day free vs. <100/month expected (verified 2026-08-13). Resend rejected on a fact: EU residency is Pro-only at $20–35/mo. **Scope is wider than ADR-023 stated** — built-in auth views make password reset email-only too. Creates 7.21, 7.22, 5.24, 11.16; DPA via 11.6 |
...
| 10.18 | Record the invitation and password-reset POSTs as explicit exclusions from [ADR-021](docs/decisions.md)'s p95 < 500 ms page budget, with the reason | Not started | [ADR-028](docs/decisions.md). Both send SMTP inline: there is no task queue, and the same ADR declined Redis, so adding Celery for a few dozen emails a month would reintroduce it. A documented carve-out for two rare, user-initiated endpoints is the cheaper answer — but it must be written down, or 7.20 reports it as a regression |
...
| 11.6 | Put DPAs in place with each third-party processor: hosting provider, Databricks, object storage, email provider, **error tracking** | Blocked | Hosting provider resolved (12.1 ✅ Railway — execute via 12.7); **Databricks resolved** (9.20 ✅ / [ADR-036](docs/decisions.md)) — Databricks Inc. *and* AWS EU (Ireland) both need DPAs, tracked in 11.17; **email provider now a real processor, not hypothetical** — [ADR-023](docs/decisions.md), choice in 9.22 (✅ Brevo). **Sentry added** by [ADR-031](docs/decisions.md) (9.15 ✅) — EU region, but a processor that by design receives whatever a traceback contains, which is why 7.23's scrubbing is part of the DPA story rather than separate from it |
...
| 11.13 | Confirm backups are encrypted at rest — **client-side `age` before upload**, so the storage processor holds only ciphertext | Not started | §6. **Unblocked** by [ADR-031](docs/decisions.md) (9.16 ✅): this is now a verification task, not an open question. Confirms 3.70/3.72 are actually doing what the ADR says; R2's own server-side encryption is a second layer, not the answer relied on |
...
| 11.16 | Add Brevo to the [gdpr.md](docs/gdpr.md) §7 processor table — EU hosting (France/Belgium/Germany), ISO 27001:2022, DPA reference, and what personal data actually reaches it | Not started | [ADR-028](docs/decisions.md) (9.22 ✅) turns §7's hypothetical email row into a real one. The data reaching Brevo is narrow but real: recipient email address, display name, and the reset/invite token. **No transfer safeguard needed** — EU-owned and EU-hosted, unlike 11.15's Railway question. Execute the DPA via 11.6 |
| 11.17 | Add Databricks Inc. and AWS (EU Ireland) to the §7 subprocessor list, with DPAs, and confirm the extract carries no personal data | Not started | [ADR-036](docs/decisions.md). Both processors sit in the EEA, so the **data** needs no Chapter V transfer mechanism; Databricks being US-headquartered is a subprocessor/DPA question, not a data-location one. Pairs with 16.3, 16.4, 16.9 |
...
| 12.11 | Set `python manage.py migrate` as Railway's **pre-deploy command**, so it completes before the new version takes traffic | Not started | [ADR-031](docs/decisions.md) (9.14 ✅). This is what makes platform-native git-watch sufficient and satisfies [ADR-015](docs/decisions.md) rule 4 without a bespoke release script. **Pairs with 7.11** — the `migrate &&` must come out of the `Dockerfile` `CMD` and the compose command in the same pass, or migrations run twice and every replica races |
...
| ID | Task | Status | Notes / source |
|---|---|---|---|
| 13.1 | Add `django-storages[s3]`, `boto3` **and `Pillow`** to `requirements/base.in`, each with an owner comment, then recompile | Not started | [ADR-005](docs/decisions.md). `Pillow` re-enters here: [ADR-029](docs/decisions.md) removed it in 19.2 because nothing imported it, and 13.4/13.5 are what make it load-bearing — `ImageField` will not validate without it |
...
| ID | Task | Status | Notes / source |
|---|---|---|---|
| 14.1 | Implement the agreed export format — a ZIP of one CSV per tenant-scoped table plus a `manifest.json` (timestamp, schema version, columns, types, PK, FK map) | **Unblocked** | [ADR-035](docs/decisions.md) (9.19 ✅). Explicitly **not** `pg_dump`, not hand-generated ANSI SQL, not `dumpdata`. Money and quantities are written at **full stored precision**, never [ADR-018](docs/decisions.md)'s 2-dp presentation rounding — an export that rounds is an export that silently loses data |
...
| 14.6 | Expose the export twice from one service function: an Owner-only download in the settings area, and a management command | Not started | [ADR-035](docs/decisions.md)/[ADR-023](docs/decisions.md), gated by an [ADR-020](docs/decisions.md) capability name rather than a role comparison. Synchronous is adequate at today's scale — **re-check it here**, since a tenant with years of goods receipts is not today's catalogue |
...
An external batch-processing service that reads a nightly extract of app data and returns margin alerts
and analytics ([ADR-002](docs/decisions.md), confirmed and scoped by [ADR-036](docs/decisions.md)).

**Databricks Serverless Jobs on AWS, EU (Ireland).** Django pushes a personal-data-free costing extract
to Cloudflare R2 and the Spark job reads only that — it never reaches PostgreSQL. Results come back to a
shared-secret callback. Hard cost ceiling **~$15/mo**, which takes total infrastructure from ~$6 to ~$21/mo
against zero revenue ([ADR-016](docs/decisions.md)) — a funded bet, so 16.8 measures a real run before
production spend.

| ID | Task | Status | Notes / source |
|---|---|---|---|
| 16.1 | Confirm the processing engine and workspace cost model before any spend | **Done** (2026-08-26) | **Spark on Databricks confirmed** — [ADR-002](docs/decisions.md) moves `Proposed` → `Accepted`, detailed by [ADR-036](docs/decisions.md): Serverless Jobs on AWS EU (Ireland), hard ceiling ~$15/mo, chosen over [ADR-024](docs/decisions.md)'s scheduled-query null hypothesis as a deliberate bet on headroom. The *measured* cost is still owed — 16.8 records one real run before production spend |
| 16.2 | Define the API contract: what the service reads, on what schedule, and what it returns | **Rescoped** | [ADR-036](docs/decisions.md): **no event triggering**. The contract is a nightly R2 extract in (16.9, 16.13) and a POST to a shared-secret callback out (16.10). What remains here is the *payload* shape of the returned insights |
| 16.3 | Define and enforce the data scope shared with the service — limited to product/pricing/recipe data, excluding user accounts and supplier contact personal data unless proven necessary | **Settled in principle** | [ADR-036](docs/decisions.md): the extract carries **product, date, cost, price, margin, tenant — and no personal data**, and Databricks never reaches PostgreSQL. The scope is therefore enforced by what the extract job writes (16.9), not by a read-role grant that could widen later. [gdpr.md](docs/gdpr.md) §7 |
...
| 16.8 | Provision the Databricks workspace — **Serverless Jobs, AWS, EU (Ireland) `eu-west-1`** — and record the **measured** DBU + cloud cost of one real run before any production spend | Not started | [ADR-036](docs/decisions.md). The DBU rate is deliberately absent from the ADR: serverless pricing is region- and tier-dependent and changes. Measure, then confirm the ceiling holds |
| 16.9 | Build the nightly extract job: a management command writing a **personal-data-free** costing extract (product, date, cost, price, margin, tenant) to a dedicated Cloudflare R2 prefix | Not started | [ADR-036](docs/decisions.md)/[ADR-005](docs/decisions.md). This job **is** the data-scope enforcement for 16.3 — Databricks never reaches PostgreSQL. Reuses [ADR-028](docs/decisions.md)'s batch costing service |
| 16.10 | Build the results callback: a plain `JsonResponse` endpoint authenticated by shared secret, tenant-scoped and rate-limited | Not started | [ADR-036](docs/decisions.md)/[ADR-028](docs/decisions.md). **The app's only internet-facing unauthenticated write path** — and the one route besides the cover and login pages exempt from [ADR-027](docs/decisions.md)'s `LoginRequiredMiddleware` (2.17) |
| 16.11 | Alert on Databricks job failure | Not started | [ADR-036](docs/decisions.md). **The real operational risk:** insights that stop arriving look exactly like "no alerts this week". Part of this feature, not of Epic 7's monitoring |
| 16.12 | Configure the spend alert at the ~$15/mo ceiling, and define what happens when it trips | Not started | [ADR-036](docs/decisions.md). Total infrastructure goes ~$6 → ~$21/mo against [ADR-016](docs/decisions.md)'s zero revenue — the ceiling is the control that keeps that a decision rather than a surprise |
| 16.13 | Version the extract schema as a published contract, and make the notebook reject an unknown version | Not started | [ADR-036](docs/decisions.md). The Django job and the Spark notebook live in different repositories and evolve separately; an unversioned format fails silently into wrong insights |
...
| ID | Task | Status | Notes / source |
|---|---|---|---|
| 17.1 | Model goods receipts as `GoodsReceipt` (supplier, receipt date, doc ref) + `GoodsReceiptLine` (material, supplier lot, quantity, unit, best-before, price paid), distinct from the `RawMaterial` catalogue row | **Unblocked** | [ADR-033](docs/decisions.md) (9.21 ✅). Header + lines because a delivery note is one record with many lines. Price fields use [ADR-018](docs/decisions.md)'s `purchase_price`/`pack_quantity`/`purchase_unit` shape — same definition as 3.10. **A lot is an event, not an attribute** |
| 17.2 | Model production runs: `ProductionRun` → `Consumption` (FK to a specific `GoodsReceiptLine`, quantity consumed in canonical units) → output `Batch` | **Unblocked** | [ADR-033](docs/decisions.md). `Consumption.quantity_consumed` is load-bearing beyond traceability — it is the single field that keeps quantity-on-hand derivable later without a schema change (17.14) |
| 17.3 | Assign an internal lot code to each produced batch, with the code format decided rather than improvised | **Unblocked** | **`YYYYMMDD-NNN`**, sequence resetting daily, unique on `(bakery, lot_code)` — [ADR-033](docs/decisions.md). Implementation is 17.13: allocated server-side inside the batch-creating transaction, never client-side and never by counting existing rows |
...
| 17.13 | Implement the lot-code generator: `YYYYMMDD-NNN`, per tenant, allocated inside the transaction that creates the batch, with `unique_together (bakery, lot_code)` as the backstop | Not started | [ADR-033](docs/decisions.md). Two runs finishing in the same second must not collide. **Never** allocate by counting existing rows; gaps after a rollback are acceptable, duplicates are not |
| 17.14 | Record `quantity_consumed` on every `Consumption` row in canonical units, so quantity-on-hand stays derivable — and deliberately surface **no** stock figure this round | Not started | [ADR-033](docs/decisions.md)/[ADR-024](docs/decisions.md). This one field is the whole content of "design so it *could* be derived later". The pilot will ask for stock: the answer is that the data is being recorded and the feature is not being claimed until it can be trusted |
...
| 19.2 | Rewrite `requirements.txt`: Django 5.2 LTS, `psycopg[binary]` 3, current `whitenoise`/`django-environ`, `gunicorn`; remove `asgiref`, `sqlparse`, `pytz`, `environ`, `dj-database-url`, `django-mathfilters`, **and `Pillow`** | Not started | [ADR-025](docs/decisions.md), amended by [ADR-029](docs/decisions.md) — **seven** removals, not six: `Pillow` has no consumer (no `ImageField` anywhere) and returns in 13.1. Leaves **five** direct dependencies. Re-save as UTF-8 in the same pass — closes 1.4, since the file is being rewritten anyway. 19.12 then converts this single file into the `requirements/` split |
...
| 19.12 | Convert the single `requirements.txt` into `requirements/{base,dev,prod}.in` and generate pinned, hashed `.txt` files with `uv pip compile`; delete the root `requirements.txt` | Not started | **Unblocked 2026-08-16** by 9.8 ✅ / [ADR-029](docs/decisions.md). Compile `base` first, then `dev` and `prod` with `-c requirements/base.txt`; `--generate-hashes --python-version 3.13` throughout. Every `.in` line gets an owner/purpose comment. `prod.in` is just `-r base.in` for now — that is the intended shape, not an omission. `dev.in`'s contents are now known (9.17 ✅ / [ADR-034](docs/decisions.md)): `ruff`, `pytest`, `pytest-django`, `pytest-cov`, `djlint`. First-release milestone item |
...
| 19.16 | Change the `Dockerfile` to install a **pinned** `uv`, then `uv pip install --system --require-hashes -r requirements/prod.txt` | Not started | [ADR-029](docs/decisions.md). Same lines 19.3 touches for the `python:3.13-slim` base image — do both in one pass. `--require-hashes` is what turns the pins into an actual integrity check; `uv` is build tooling and never enters `base.in` |
| 19.17 | Add a lock-drift check to CI: recompile the `.in` files and fail the PR if the committed `.txt` files differ | Not started | [ADR-029](docs/decisions.md). Extends 1.11 (or 6.12 if Epic 6 lands first). Without it the generated files are only as fresh as the last person who remembered, which is the failure the `.in`-as-register rule exists to prevent |
...
|---|---|
| [ADR-002](docs/decisions.md) — Spark/Databricks insights service (**Accepted**; confirmed, scoped and costed by ADR-036) | 9.20 ✅, 16.1 ✅, 16.2–16.13 |
...
| [ADR-030](docs/decisions.md) — frontend: shared `base.html` + partials, Bootstrap 5.3, HTMX, no SPA, **no build step**, `djlint` | 9.13 ✅ and the template half of 9.17; unblocks 5.4, 5.6, 5.7, 5.8; **rescopes** 5.6 (consolidate CSS, no pipeline); sharpens 5.1, 5.2, 5.3, 5.5; new 5.25 |
| [ADR-029](docs/decisions.md) — `uv pip compile` over a three-file `requirements/` split; the `.in` file is the dependency register | 9.8 ✅, 19.12 (unblocked), new 19.16 and 19.17, the extra `Pillow` removal in 19.2, and its re-addition in 13.1 |
| [ADR-031](docs/decisions.md) — operations: coverage-gated CI, Railway git-watch deploys with a pre-deploy migrate, Sentry EU, UptimeRobot, two-track backups with no PITR | 9.14 ✅, 9.15 ✅, 9.16 ✅; new 3.70, 3.71, 3.72, 6.22, 7.23, 12.11; the **unblocking** of 3.38, 3.40, 6.12, 7.2, 7.5, 7.15, 11.13; the **rescoping** of 6.14 (branch protection *is* the deploy gate), 3.48 (weekly, must decrypt) and 12.9 (rollback track only); and Sentry added to 11.6 |
| [ADR-028](docs/decisions.md) — backend architecture: settings layout, batch-first service layer, no API, permissions on Django's own rails, no cache, Brevo over SMTP | 9.9 ✅, 9.10 ✅, 9.11 ✅, 9.12 ✅, 9.22 ✅ and the caching half of 9.18; new 2.19–2.23, 4.19, 4.20, 5.24, 7.21, 7.22, 10.18, 11.16; the **rescoping** of 1.5 (three settings modules) and 4.1, the **unblocking** of 4.5 and 5.19, the **cancellation** of 4.15, and the **supersession** of 2.10 by 4.5 + 2.23 |
| [ADR-032](docs/decisions.md) — one shared `RecipeLine`; categories as a tenant-scoped lookup table with a `kind` discriminator | 9.18 ✅; the **unblocking** of 3.19 and the categories half of 3.26; new 3.73, 3.74; and the **correction** of 3.42 (superseded by 7.19) and 4.14 (cancelled), which both still cited 9.18 |
| [ADR-033](docs/decisions.md) — traceability entities: receipt headers with lines, production runs emitting batches, `YYYYMMDD-NNN` lot codes, stock derivable but not surfaced | 9.21 ✅; the **unblocking** of 17.1, 17.2, 17.3; new 17.13, 17.14; the confirmation of 17.12; and a **negative** constraint on 3.10/3.14 (no `lot` field, no `stock` field) |
| [ADR-034](docs/decisions.md) — `ruff` for lint *and* format; `pytest-django` as the test runner | 9.17 ✅; the **unblocking** of 6.10 and 6.11; new 6.23, 6.24; feeds 6.12 and 6.22 |
| [ADR-035](docs/decisions.md) — tenant export as a CSV bundle + JSON manifest in a ZIP | 9.19 ✅; the **unblocking** of 14.1; new 14.6; sharpens 14.3 (the manifest is the deliverable) and 14.4 |
| [ADR-036](docs/decisions.md) — insights confirmed on Databricks Serverless (AWS EU Ireland), R2 extract handoff, nightly schedule, ~$15/mo ceiling; gunicorn/WSGI stands | 9.20 ✅, 9.4 ✅, 16.1 ✅; the **unblocking** of 7.14, 16.2, 16.3; new 16.8–16.13, 11.17; and the **rescoping** of 16.2 (no event triggering) |
...
| Backups that have never been restored — the restore path currently runs for the first time on the day it matters, over records food law requires to be kept | 3.70 (the portable dump exists at all), 3.48 (weekly drill), 3.72 (the key that decrypts it) |
...
| Schema redesigned in Epic 3 without accounting for Epic 17's event layer, forcing a second redesign | **9.21 ✅ decided 2026-08-26** ([ADR-033](docs/decisions.md)), before 3.19/3.22 are implemented. Epic 3 now builds `RawMaterial` knowing the event tables are coming: no `lot` field, no `stock` field, numeric quantity with a unit FK |
...
- [ ] Supported Python and Django versions, with pinned, reproducible dependencies (Epic 19; lock workflow in 19.12, decided in [ADR-029](docs/decisions.md))
