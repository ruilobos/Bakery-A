...
| 1.7 | Rename prototype identifiers: `categorie`→`category`, `recipe_yeld`→`recipe_yield`, `Base_recipes`→`BaseRecipe`, `Bs_Ingredients`→`BaseRecipeIngredient`, `Recipe_Ingredients`→`ProductIngredient` | Blocked | **Blocked on [roadmap.md](docs/roadmap.md) 9.23**, opened 2026-09-18 when `/next-task` picked this row up — ADR-012: a task blocked on an open question is `Blocked` with a pointer. Two findings, one cause. **The Notes cited no ADR**, which ADR-038 makes a defect rather than a task to start. And ADR-032 **deletes four of these five identifiers instead of renaming them**: 3.73 merges `Bs_Ingredients` + `Recipe_Ingredients` into `RecipeLine` ("*the migration is a merge, not a rename*"), 3.74 drops the `categorie` column for a tenant-scoped `Category` table, and 3.60 retypes `recipe_yeld` per ADR-022. Only `Base_recipes`→`BaseRecipe` survives Epic 3, which keeps `Product` and `Base_recipes` separate. Measured 2026-09-18: `categorie` 124 sites / 42 files, `Recipe_Ingredients` 46 / 10, `Base_recipes` 34 / 10, `recipe_yeld` 27 / 13, `Bs_Ingredients` 25 / 7. **The task text is left as written** — rescoping it is 9.23's answer, not a backlog edit. Original intent retained for whoever answers it: transitional migrations + compatibility layers, never a big-bang rename; sequence with Epic 3 |
...
| 1.15 | Guard hooks: block direct pushes to `main`/`production`, block a drive-by rename of the prototype identifiers, block a commit that leaves this file's task status stale | Not started | ADR-038. Local only, so they are a fast failure and never the real enforcement — that stays 1.9 and 1.11. The rename guard exists because `categorie` alone is 124 sites across 42 files (re-measured 2026-09-18; the earlier 137/58 predated 1.1–1.6) — and 1.7 is now `Blocked` on 9.23, so the guard is what keeps those sites intact while the question is open |
