...
|---|---|---|---|
| 1.1 | Remove generated and local-only artifacts from version control (`__pycache__/`, `bakery/staticfiles/`, `.idea/`, `.venv/`, OS/editor temp files) | **In progress** | PR #5 untracked 95 files — 88 `__pycache__` (including stale `cpython-38` bytecode), `.idea/`, `.vscode/settings.json`; tracked files 302 → 207, nothing deleted from disk. **`bakery/staticfiles/` (60 files) deliberately left** — untracking it is gated on 1.3's check, so this closes when 1.3 does |
...
| 1.3 | Keep only source assets in `bakery/static/`; confirm nothing depends on the committed `staticfiles/` output, then untrack it | Not started | **Now also owns untracking the 60 `bakery/staticfiles/` files** — 1.1 deferred them here rather than removing them blind (31 templates hardcode `/static/`; the `Dockerfile` runs `collectstatic` at build). Add the `.gitignore` pattern in the same pass; 1.1 closes with this. **Prerequisite for 5.3** — a committed stale manifest is the classic "worked locally, 500 in production". ADR-027 |
