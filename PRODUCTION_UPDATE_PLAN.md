...
| 1.6 | Remove dead code and unused views/forms that no later task already owns — today that is `control/forms.py` in its entirety | Not started | **Scope is narrow by subtraction, and that is the point.** Verified 2026-09-15: `Raw_Material_Form` is referenced nowhere, and the module's `Recipe_Ingredients` import is unused too, so the file goes whole. **Everything else that looks dead belongs to another task — do not sweep it here:** the unreachable `accounts/` auth code is **4.5** (ADR-028), the seven 0-byte JS files are **5.5**, the duplicate CSS is **5.6**, Bootstrap's RTL/`.map` builds are **5.4**, and identifier renames are **1.7**. **Trap:** `accounts/templates/registration/logged_out.html` reads as orphaned to a grep because nothing names it — it is Django's `LogoutView` convention, and ADR-028 requires keeping that directory. A dead-code sweep deletes it. ADR-028 |
