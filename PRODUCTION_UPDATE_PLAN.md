...
|---|---|---|---|---|---|
| [1](#epic-1--stabilize-the-repository) | Stabilize the repository | `phase-1-repo-cleanup` | In progress | 20 | — |
...
|---|---|---|---|
| 1.1 | Remove generated and local-only artifacts from version control (`__pycache__/`, `bakery/staticfiles/`, `.idea/`, `.venv/`, OS/editor temp files) | **Done** 2026-09-15 | PR #5 untracked 95 files — 88 `__pycache__` (including stale `cpython-38` bytecode), `.idea/`, `.vscode/settings.json`. The remaining 60 `bakery/staticfiles/` files were deferred to 1.3 rather than removed blind, and **closed with it**. Tracked files 302 → 153 across both, net of the seven `.claude/` files 1.14 added in between; nothing deleted from disk |
| 1.2 | Expand `.gitignore` to cover Python, Django, environment, build and IDE artifacts | **Done** 2026-09-10 | PR #5. Replaced three lines with Python/Django/env/IDE/OS coverage, including `~$*` after a Word owner file (`~$quirements.txt`) reached `main` in PR #2. Verified by running `manage.py check` and confirming the tree stays clean — previously that alone dirtied 19 files. `bakery/staticfiles/` is intentionally absent: the pattern would be inert while the files are tracked, so it lands with 1.3 |
| 1.3 | Keep only source assets in `bakery/static/`; confirm nothing depends on the committed `staticfiles/` output, then untrack it | **Done** 2026-09-15 | Untracked the 60 `bakery/staticfiles/` files and added the `.gitignore` pattern; **closes 1.1**. **The confirmation passed decisively:** 46 distinct static references across 58 project files — absolute *and* relative, since `core/templates/cover.html` uses `static/…` without a leading slash — **all** resolving against a freshly regenerated output. The committed output was a **stale partial artifact nothing read** — no `admin/`, no `staticfiles.json`, 60 files where `collectstatic` produces 128; `Dockerfile:20` regenerates it at build. Four unreferenced orphans existed only in the output dir (incl. a 380 KB `.pptx`) and went with it — `Logo1.png` remains in source. `bakery/static/` left untouched: its 0-byte JS and Bootstrap RTL/`.map` files belong to 5.5 and 5.4, and ADR-030 makes the vendored assets load-bearing. **Prerequisite for 5.3** — a committed stale manifest is the classic "worked locally, 500 in production". ADR-027 |
...
| 1.5 | Split settings into `settings/base.py` + `local.py` + `test.py`; retire the Heroku module | **Done** 2026-09-17 | ADR-028: **three** modules, not four — `base.py` *is* production. `manage.py`/`wsgi.py`/`asgi.py` keep defaulting to `base`, and the `Dockerfile` now states `DJANGO_SETTINGS_MODULE` explicitly. **Structure only:** every environment-sensitive value reads from the environment via `django-environ`, but today's values stay as `default=` fallbacks so behaviour is unchanged — **2.1** removes the secret fallbacks and **2.19** removes them entirely, at which point `ImproperlyConfigured` is raised by the library, so 2.19 is "delete the fallbacks", not "write a validator". Fallbacks are also what keeps `collectstatic` building while **9.22** is open. **Deleted `settings/heroku.py` only** — `runtime.txt` is 19.3's (ADR-025 ties the Python version to the Dockerfile) and the `Procfile` is 7.9's. **Trap:** `manage.py runserver` alone now gives `DEBUG = False`; local work needs `DJANGO_SETTINGS_MODULE=bakery.settings.local` until 1.12 lands |
| 1.6 | Remove dead code and unused views/forms that no later task already owns — `control/forms.py` in its entirety, plus the unused imports in `control/views.py` and `control/urls.py` | **Done** 2026-09-17 | Deleted `control/forms.py` (`Raw_Material_Form`, referenced nowhere — no import, no string reference, no `form_class` on any view; every view uses `fields = '__all__'`) and removed **7 verified-unused imports**: `get_object_or_404`, `HttpResponseRedirect`, `DetailView`, `reverse`, `datetime`, `staff_member_required` from `views.py`, and `admin` from `urls.py`. The imports were outside the original Notes because the check behind them looked only for unused *forms*; `reverse_lazy` (15 uses) is imported separately and untouched. **Trap, recorded because it nearly bit:** `get_item` in `views.py` is a **registered template filter**, not an unrouted view — a "find unrouted views" sweep flags it, but `dashboard.html` and `base_recipe.html` use it, and it only registers as a side-effect of importing `views.py`. It stays. **Finding for 2.17:** `staff_member_required` was imported and never used — the only trace in the code that someone meant to guard these views, which matches the inconsistent access control 2.17 fixes. The dead import went; the observation is recorded here. Scope subtracted as before: `accounts/` → 4.5, 0-byte JS → 5.5, duplicate CSS → 5.6, Bootstrap RTL/`.map` → 5.4, renames → 1.7, `tests.py`'s stub import → 6.23. ADR-028 |
| 1.7 | Rename the two prototype identifiers Epic 3 leaves behind: `Base_recipes`→`BaseRecipe`, and `recipe_yeld`→`recipe_yield` on both models | **Done** 2026-09-22 | ADR-039. **Rescoped 2026-09-21 from five identifiers to two.** ADR-032 deletes the other three rather than renaming them — 3.74 drops the `categorie` column, 3.73 merges `Bs_Ingredients` + `Recipe_Ingredients` out of existence — but it renames **nothing**: it keeps `Base_recipes` a separate model, and 3.60 retypes `recipe_yeld` without respelling it, so without this task both misspellings ship. **Plain `RenameModel`/`RenameField`, no transitional columns or shims** — the original "compatibility layers" note cited no ADR and guarded against side-by-side code versions that cannot exist while there is no host (ADR-039's Rejected). **Data layer only: 6 files and one migration** — `models.py`, `views.py`, `admin.py` and the `base_recipe`/`products`/`new_product` templates. The `Base_recipes_*` **view classes stay as they are** until 4.3, which keeps `urls.py` out of the diff. Traps in ADR-039 — the string FK on `Bs_Ingredients.base_recipe`, `recipe_yeld` on two models, and `export_products`, which is separately broken (1.17). **Implementing it also restates prose that names the old spellings**: `project_requirements.md`'s base-recipes and soft-delete rows, and CLAUDE.md's quirks list, which loses two of its five entries. **Landed as predicted: 6 code files + `0013_rename_base_recipes_recipe_yeld.py`, and `urls.py` untouched.** The migration was **hand-written** — `makemigrations` asks interactively whether a rename occurred, and `--no-input` answers *no*, emitting `DeleteModel` + `CreateModel`, which drops the table. Proof it matches the models is `makemigrations --check --dry-run` reporting no changes. The rename was word-boundary anchored (`\bBase_recipes\b`): a global substitution would have caught `Base_recipesList` and the three `Base_recipes_*` view classes and pulled `urls.py` in, which `codecheck` flagged before any edit. `RenameModel` takes the table `control_base_recipes` → `control_baserecipe`; no `db_table` pin, since pinning would keep the misspelling in the schema. **Found on the way and filed, not fixed: 1.20** |

### Branch, release and CI plumbing

| ID | Task | Status | Notes |
|---|---|---|---|
| 1.8 | Create the `production` branch as the deploy branch, with `main` as integration/dev-test | **Done** 2026-09-22 | ADR-010, ADR-041. **"Create" was wrong: the branch already exists** — `origin/production` at `96faf98`, *"Update README.md"*, **2021-06-30** — so this is an **adopt**, not a create. The premise gate measured it a **strict ancestor of `main`**, 40 behind with **zero commits of its own** (`git rev-list --left-right --count origin/main...origin/production` → `40  0`), which is what makes advancing it a plain fast-forward: no history discarded, **no force-push**, and the protection already on the branch holds. **The baseline merge is deliberately untagged** — ADR-041. ADR-010 says every merge to `production` is tagged semver and published as a Release, but 1.10 has yet to define that step, `.github/` does not exist and there is no host, so a `v0.1.0` would assert something is live when nothing is. Tagging starts at the first genuine promotion. **Two PRs, and the order and merge method both matter:** the docs PR squash-merges to `main` first, then the `main` → `production` PR is merged with **"Create a merge commit", never squash** — squashing would stop `production` being a descendant of `main` and break every later fast-forward. Status stays `In progress` until that second PR merges. **Found on the way and filed, not fixed: 1.9's deliverable is already configured on GitHub** — PR-only, `enforce_admins`, no force-pushes, no deletions, 0 approvals, no status checks, on *both* branches — while its row still reads `Not started`; correcting it is its own PR. The README is still the 2021 prototype text and documents no branch model; also left alone. **Landed as PR #19 (docs → `main`) and #20 (`main` → `production`), both merged 2026-09-22.** `production` moved `96faf98` → `b39c31e` and its tree is now **identical to `main`** (`git diff origin/main origin/production` → empty), with `main` a strict ancestor of it (`git merge-base --is-ancestor origin/main origin/production` → exit 0) — the descendant property ADR-041 protects. **Zero tags and zero Releases**, as intended. **One prediction was wrong and is worth recording:** the plan expected `git rev-list --left-right --count origin/main...origin/production` to read `0  0` after the merge; it reads `0  1`. The extra commit is the merge commit itself — GitHub's "Create a merge commit" always writes one even where a fast-forward was possible, so `0  0` was only ever reachable by the squash this task forbids. The state is correct; the expectation was not |
| 1.9 | Branch protection on `main` and `production`: PR-only, no direct pushes, no force-pushes (0 required approvals while solo) | **Done** 2026-09-22 (verified; configured earlier) | ADR-010. **The work was already done when this row said `Not started`** — found by the premise gate during 1.8 and verified clause by clause against the live GitHub API before this status was changed. Classic branch protection, **byte-for-byte identical on both branches**: `required_pull_request_reviews` present (PR-only), `allow_force_pushes: false`, `allow_deletions: false`, `required_approving_review_count: 0` with `require_code_owner_reviews` and `require_last_push_approval` both false. **`enforce_admins: true` is the clause that makes it real** — without it a repo admin can push straight past protection, so the solo owner would have been the one person it did not bind; a direct push to either branch is now rejected server-side. **No `required_status_checks`** — correct, not an omission: ADR-010 defers them to "once Epic 6 exists", and 1.11 has yet to create a check to require. **No rulesets** (`gh api repos/{owner}/{repo}/rulesets` → `[]`), so classic protection is the whole of the enforcement. **Two honesty notes.** This task has **no PR of its own**, so it does not meet this file's "`Done` when its own PR merges" rule — it was configured directly on GitHub outside the task loop, and the date above is when it was *verified*, not when it was set; **the configuration date is not recoverable** (the protection payload carries no timestamp and the audit-log API is Enterprise-only, so both endpoints 404). Recorded rather than guessed. This is exactly the drift ADR-040's gate exists to catch |
| 1.10 | Define the release step: every merge to `production` is tagged semver and published as a GitHub Release with auto-generated notes | **Done** 2026-09-24 | ADR-010, ADR-041, ADR-042. **`.github/workflows/release.yml`**: `publish` tags every push to `production` and creates the Release with GitHub's generated notes, using only the job's own token. `check` validates an open release PR (head `main`, title `Release vX.Y.Z`) and is advisory until 6.13, whose Notes now name it. **The version rule is `scripts/release_version.py`**, with 14 tests in `scripts/test_release_version.py`: the title must name one of the three successors of the latest `v*` tag, and the first tag is `v0.1.0`. **Nothing exercises `publish` before the next release PR**, because a push event reads the workflow from the pushed commit. Its first live run is the next promotion, titled `Release v0.1.0` and merged with a merge commit (ADR-041). **Proven before merge:** the tests (three deliberate mutants caught); actionlint and shellcheck clean; the rule dry-run against the live tags; and, against `production`, the release-PR lookup and the generated-notes preview. 1.11's "(none exist)" was made untrue by these tests and now gives ADR-011's reason instead. **Found on the way, not fixed** (each its own PR): ADR-041's Decision, index row and ancestor-property Trap, and the first half of 1.8's Notes, still describe a fast-forward where PR #20 wrote a merge commit; **041 starts 014's release gate at the first tagged promotion, which will likely precede any Railway project (12.5)**, so this is a roadmap question to settle before `Release v0.1.0` is opened; ADR-031's SAST revisit trigger, "repo goes public", has fired or was miswritten, since the repo is public; `project_requirements.md` Part 2 has no rule keeping production data out of PR titles, commit messages and release notes; 7.12, 8.4, 3.29 and 3.33 cite no ADR; tags are unprotected and immutable releases are off; `production` still allows squash and rebase merges; the Actions default token has write access and may approve PRs; PR #21 reached `main` as a merge commit, not a squash |
| 1.11 | Minimal GitHub Actions workflow on every PR against `main`/`production`: `manage.py check`, `makemigrations --check --dry-run`, `docker build`, basic lint | Not started | ADR-011. No test run (ADR-011 leaves it to Epic 6) and no deploy step (no host yet) — verification only; extended by 6.12 |
...
| 1.18 | Add the `codecheck` subagent — owns the working tree and git history, never `docs/` — and the `/next-task` premise gate that calls it before planning; add `Bash(git grep:*)` to the permission allow list | **Done** 2026-09-21 | ADR-040. The gate re-derives four claim types: a number, a "superseded by task N", a constraint, and the task's purpose. **Before planning, not before implementing** — a premise checked after the branch exists has already cost what it was meant to save. `codecheck` returns the **command** with every measurement, matches identifiers on **word boundaries** (`categorie` matches `categories`) and separates model / field / view-class / URL-name / template-variable hits, which are the two rules that caught ADR-039's errors. `settings.json` allows `Bash(grep:*)` but not `git grep`, so the measuring agent would prompt on every call. **Landed as** `.claude/agents/codecheck.md`, SKILL.md's new **step 4** (steps 4–12 renumbered 5–13), and the allow-list entry. **One correction inside the blast radius:** the plan-mode note under step 5 pointed at ADR-038's traps "for the scale of 1.7" — the figure ADR-039 corrected — so it now cites ADR-039; leaving the loop's own instructions quoting the number that motivated the gate would have been self-contradictory. **No tests:** there is no runner until 6.23, and these are agent/skill definitions; verified by loading them (`/agents`, `/next-task`) and by `python -m json.tool` on `settings.json` |
| 1.19 | Have the `decisions` and `backlog` agents flag claims about the code as unverified, and add CLAUDE.md's "docs state intent; the code states fact" rule | **Done** 2026-09-21 | ADR-040. **Neither agent gains code access** — that would dissolve the isolation ADR-038 bought; they mark numbers and file counts *stated when written, never re-measured* and hand off to `codecheck`. Also amends `decisions`' "do not reconstruct one from the code" so the dead end becomes a handoff: an unsettled question is only a blocker if its premise holds. Depends on 1.18 for the agent to hand off to. **Landed** as one new Rule in each agent plus the reworded "reconstruct" line, and CLAUDE.md's new `### Docs state intent; the code states fact` between the `docs/` overview and the workflow section. `backlog.md` already told the agent to report a file that is "stale or self-contradictory", so the new rule names the distinction explicitly — that one is the file disagreeing with *itself*, this one is the file disagreeing with the *codebase*. **Premise-checked with `codecheck`** (its first run): all six claims in these Notes verified against the files before any edit |
...
| 1.20 | Remove the two unreachable, broken `@property` methods on `Recipe_Ingredients` — `margin_percent` and `margin_value` | Not started | **Broken since 2021**, found by `codecheck` during 1.7's premise gate 2026-09-22. Both read `self.recipe_yield`, `self.ingredients`, `self.price` and `self.vat`; `Recipe_Ingredients` has **none** of the four, so either raises `AttributeError` on any call. **Unreachable**: `dashboard.html` and `products.html` read the context dicts built in `views.py`, never these properties. Copy-paste from `Product` in the prototype. **1.6 owned dead code and missed them** — its sweep looked for unused forms and imports, not for properties referencing absent fields. 1.7 renamed the identifier inside them without touching the defect, per the no-drive-by rule. **Delete, do not repair** — ADR-018's trap is the rule ("the inline `float()` costing is now definitively wrong, not merely duplicated … **Deleted, not patched**"), and ADR-032 puts the host model itself on the way out: 3.73 merges `Recipe_Ingredients` into `RecipeLine` and drops it, so repaired margin math here would survive nothing. 4.1 separately deletes the model `@property` methods wholesale when costing moves into `control/services/` |
...
| 3.2 | Tenant FK on every business table: `Supplier`, `RawMaterial`, `BaseRecipe`, `Product`, `RecipeLine`, future `Profile` | Not started | ADR-008 |
...
| 3.22 | Soft delete / archive on `Supplier`, `RawMaterial`, `Product`, `BaseRecipe`; ingredient lines stay hard-deletable | **Unblocked** | ADR-019 — "anything referenced by a record that outlives it" |
...
| 3.60 | Make `recipe_yield` a real numeric quantity with a unit from 3.52 | Not started | Cost *per unit of yield* is what a parent consumes, so a loose yield field can no longer work. Extends 3.14. Field respelled by 1.7 (ADR-039); this task still owns the **type**, which is untouched — `IntegerField` on `BaseRecipe`, `DecimalField(6,2)` on `Product` |
...
| 4.3 | Use class-based or function-based views consistently | Not started | **Also owns the prototype view-class names left behind by 1.7** (ADR-039 scopes that task to the data layer): `Base_recipesList` and `Base_recipes_Create`/`_Update`/`_Delete` in `views.py`, plus their four `urls.py` call sites. The model is now `BaseRecipe`, so the views are a visibly half-done rename until this lands — deliberate, not an oversight. Renaming them is a view-structure decision, not a schema one, which is why it waited |
...
| 5.6 | **Rescoped — no build step.** Consolidate the 16 CSS files (922 lines, **10 of them byte-identical duplicates in 3 groups**) into a small set on CSS custom properties, relying on whitenoise for hashing and compression | **Unblocked** | ADR-030 **rejects the Vite candidate**. Native nesting/custom properties are safe on the evergreen matrix. Revisit trigger in the ADR. **Duplication re-measured 2026-09-15** and it is wider than the "four byte-identical" first recorded: 4 × (`base_recipe`, `dashboard`, `products`, `raw_materials`), 4 × (`edit_base_recipe`, `product`, `raw_material`, `suplier`), 2 × (`products_categories`, `rw_sectors`). The 16/922 count itself is confirmed correct — it excludes the vendored Bootstrap files |
...
| 6.13 | Enable required status checks on `main` and `production`, blocking merge when checks fail | Not started | ADR-010. **This is also the deploy gate** — see 6.14. On `production`, include 1.10's `Validate release PR` check (ADR-042) |
...
| 7.9 | Remove the remaining legacy Heroku assumption: the `Procfile` | Not started | **Narrowed 2026-09-17 so each file is claimed once.** `settings/heroku.py` was deleted by **1.5**; `runtime.txt` belongs to **19.3** (ADR-025 makes the Dockerfile the single source of the Python version). Only the `Procfile` is left here. Pairs with Epic 12, which replaces the host |
...
| 19.3 | Change the `Dockerfile` base image to `python:3.13-slim`; delete `runtime.txt` rather than updating it | Not started | ADR-025, ADR-015 rule 1 — the Dockerfile becomes the single source of the Python version. **Sole owner of the `runtime.txt` deletion as of 2026-09-17** — 1.5 and 7.9 both used to claim it too. Do in one pass with 19.16 |
...
| 042 — A workflow tags every merge to `production`, versioned by the release PR's title | 1.10, 6.13 |
